...
def roundNumber(number):
    ...
    number = str(number)
    if number[-2] == '.':
        number = number + '0'       #debugging statement using string slicing for when a value ends in one decimal place
    number = float(number)
...
def exit_dond():       #confirm that user wants to exit if user clicks the window X
    answer = messagebox.askyesno("Deal or No Deal", "Would you like to play again?")
...
lblMessage = Label(frame, width=35, bg="black", font=("Century Gothic", 14, "bold"), fg="#fcea97",
